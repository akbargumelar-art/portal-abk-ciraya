/**
 * Inject Voucher Fisik Performance Page
 * 
 * Revenue & Quantity metrics for Simpati and byU products - Inject Voucher
...
import { aktifasiData as injectData } from '../../services/mock/aktifasiData';
                    ...
                    <table className="data-table data-table-compact whitespace-nowrap">
                        <thead className="sticky top-0 z-10">
                            <tr className="bg-gray-100">
                                ...
                                <th colSpan={colCount} className="p-2 border text-center bg-red-100 font-semibold">Simpati</th>
                                <th colSpan={colCount} className="p-2 border text-center bg-purple-100 font-semibold">byU</th>
                                <th colSpan={colCount} className="p-2 border text-center bg-blue-100 font-semibold">Total</th>
                            </tr>
                            <tr className="bg-[#3d5f85]">
                                {/* Simpati */}
                                {headerCols.map(col => <th key={`s-${col}`} className="p-1.5 border text-center text-red-800 bg-red-50">{col}</th>)}
                                {/* byU */}
                                {headerCols.map(col => <th key={`b-${col}`} className="p-1.5 border text-center text-purple-800 bg-purple-50">{col}</th>)}
                                {/* Total */}
                                {headerCols.map(col => <th key={`t-${col}`} className="p-1.5 border text-center text-blue-800 bg-blue-50">{col}</th>)}
                            </tr>
                        </thead>
                        <tbody>
                            {data.map((row, idx) => {
...
const DailyTrendChart: React.FC<{ areaFilter: string, isRevenue: boolean }> = memo(({ areaFilter, isRevenue }) => {
    const trendData = isRevenue ? injectData.dailyTrendRevenue : injectData.dailyTrendQuantity;
...
// MAIN PAGE COMPONENT
// ===========================================
const InjectVoucherPage: React.FC = () => {
    ...
    const summaryData = isRevenue ? injectData.revenue : injectData.quantity;
    ...
    return (
        <div className="p-6 animate-fade-in">
            <Header title="Performansi Inject Voucher Fisik" subtitle="Revenue & Quantity Monitoring - Simpati & byU" />
...
export default InjectVoucherPage;
