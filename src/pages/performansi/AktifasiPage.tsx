/**
 * Aktifasi Performance Page
 * 
 * Revenue & Quantity metrics for Simpati and byU products
 * Features: View Switcher (Revenue/Quantity), Daily Trend Chart, Area Summary
 */
...
import { aktifasiData } from '../../services/mock/aktifasiData';
                    ...
                    <table className="w-full text-[10px] border-collapse whitespace-nowrap">
                        <thead className="sticky top-0 z-10">
                            <tr className="bg-gray-100">
                                ...
                                <th colSpan={colCount} className="p-2 border text-center bg-red-100/70 font-semibold">Simpati</th>
                                <th colSpan={colCount} className="p-2 border text-center bg-purple-100/70 font-semibold">byU</th>
                                <th colSpan={colCount} className="p-2 border text-center bg-slate-100 font-semibold">Total</th>
                            </tr>
                        ...
                        <tbody>
                            {data.map((row, idx) => {
                                ...
                                return (
                                    <tr key={row.area_name} className={`${rowBg} hover:bg-slate-50 border-b`}>
...
const DailyTrendChart: React.FC<{ areaFilter: string, isRevenue: boolean }> = memo(({ areaFilter, isRevenue }) => {
    const trendData = isRevenue ? aktifasiData.dailyTrendRevenue : aktifasiData.dailyTrendQuantity;
...
const AktifasiPage: React.FC = () => {
    ...
    const summaryData = isRevenue ? aktifasiData.revenue : aktifasiData.quantity;
    ...
    return (
        <div className="p-6 animate-fade-in">
            <Header title="Performansi Aktifasi" subtitle="Revenue & Quantity Monitoring - Simpati & byU" />
