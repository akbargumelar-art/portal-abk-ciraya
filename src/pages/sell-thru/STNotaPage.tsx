...
import { Clock, FileText, ChevronDown, ChevronUp, MapPin, User, Store, Download } from 'lucide-react';
...
const MetricCell = memo(({ metric, outletAktif, ratePjp }: {
    ...
    return (
        <>
            {/* Outlet Aktif & Rate PJP (if provided) */}
            {outletAktif !== undefined && (
                <>
                    ...
                    <td className="p-1 text-center text-[9px] bg-blue-50">
                        <span className={`px-1 py-0.5 rounded text-[8px] font-bold ${ratePjp! >= 80 ? 'bg-green-100 text-green-700' :
                            ratePjp! >= 50 ? 'bg-yellow-100 text-yellow-700' :
...
// ===========================================
// SUMMARY TABLE COMPONENT (TAP / SF)
// ===========================================
...
const SummaryTable: React.FC<SummaryTableProps> = ({ data, title, icon, mode }) => {
    ...
    const products = mode === 'perdana'
        ...
        : VOUCHER_BRANDS.flatMap(brand =>
            VOUCHER_VALIDITIES.map(validity => ({
                id: `${brand}_${validity}`,
                name: VALIDITY_LABELS[validity],
                brand,
            }))
        );

    const getMetric = (row: SummaryRow, productId: string): SellThruMetric => {
        return mode === 'perdana' ? row.perdana[productId] : row.voucher[productId];
    };

    const getOutletAktif = (row: SummaryRow, productId: string): number => {
        return mode === 'perdana' ? row.perdana_outlet_aktif[productId] || 0 : row.voucher_outlet_aktif[productId] || 0;
    };

    const getRatePjp = (row: SummaryRow, productId: string): number => {
        return mode === 'perdana' ? row.perdana_rate_pjp[productId] || 0 : row.voucher_rate_pjp[productId] || 0;
    };
    ...
    const getAggregatedMetric = (row: SummaryRow, productIds: string[]): SellThruMetric => {
        ...
        const gap = actual - target; // Gap of totals
    ...
    // 9 per product: 2 (aktif/rate) + 7 (tgt, act, ach, gap, m-1, mom, gmom)
                                ...
                                <tr>
                                    ...
                                    {VOUCHER_BRANDS.map(brand => (
                                        <th key={brand} colSpan={VOUCHER_VALIDITIES.length * metricColSpan}
                                            className={`p-2 border-b border-l-2 text-center ${brand === 'simpati' ? 'bg-red-100 text-red-800 border-red-300' : 'bg-purple-100 text-purple-800 border-purple-300'}`}>
                            ...
                            <tr className="bg-[#3d5f85]">
                                ...
                                {products.map((product, idx) => (
                                    <th key={product.id} colSpan={metricColSpan}
                                        className={`p-2 border-b text-center ${idx === 0 || (mode === 'voucher' && idx === VOUCHER_VALIDITIES.length) ? 'border-l-2 border-gray-400' : 'border-l'} ${product.brand === 'simpati' ? 'bg-red-50' : 'bg-purple-50'}`}>
                                        {product.name}
                                    </th>
                                ))}
                            </tr>
                            ...
                            {data.map(row => (
                                <tr key={row.name} className="hover:bg-gray-50 border-b">
                                    ...
                                    {products.map(product => (
                                        <MetricCell
                                            key={product.id}
                                            metric={getMetric(row, product.id)}
                                            outletAktif={getOutletAktif(row, product.id)}
                                            ratePjp={getRatePjp(row, product.id)}
                                        />
                                    ))}
                                </tr>
                            ))}
...
const DetailTable: React.FC<DetailTableProps> = ({ data, mode }) => {
    ...
    const filteredData = useMemo(() => {
        ...
        return data.filter(o =>
            ...
            o.id_digipos.toLowerCase().includes(q) ||
            o.salesforce_name.toLowerCase().includes(q) ||
            o.rs_number.toLowerCase().includes(q)
        );
    }, [data, searchQuery]);
    ...
    const getMetric = (row: OutletSellThruData, productId: string): SellThruMetric => {
        return mode === 'perdana' ? row.perdana[productId] : row.voucher[productId];
    };

    const getFisikBadge = (status: string) => {
        const colors = {
            exist: 'bg-green-100 text-green-700',
            closed: 'bg-red-100 text-red-700',
            relocated: 'bg-yellow-100 text-yellow-700',
        };
        const labels = { exist: 'Ada', closed: 'Tutup', relocated: 'Pindah' };
        return (
            <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold ${colors[status as keyof typeof colors] || 'bg-gray-100'}`}>
    ...
    // 6 columns per product: Tgt, Act, Ach%, Gap, M-1, MoM
            ...
            {expanded && (
                <>
                    <div className="p-3 border-b bg-white flex items-center justify-between gap-4">
                        ...
                        <button
                            onClick={() => exportDetailOutlet(filteredData as unknown as Record<string, unknown>[], products, mode, 'nota')}
                    ...
                    <div className="overflow-x-auto max-h-[500px]">
                        <table className="w-full text-[9px] border-collapse whitespace-nowrap">
                            <thead className="sticky top-0 z-20 bg-[#2c4a6a]">
                                {/* Row 1: Product Groups (only for voucher mode) OR Product Names + Metrics */}
                                {mode === 'voucher' && (
                                    <tr>
                                        ...
                                        {aggregatedGroups.map(group => {
                                            ...
                                            return (
                                                <React.Fragment key={group.name}>
                                                    ...
                                                    <td className={`p-1 text-center font-semibold text-blue-700 ${group.bg}`}>{m.actual}</td>
                                                    <td className={`p-1 text-center ${group.bg}`}>
                                                        <span className={`text-[8px] font-bold ${m.achievement_pct >= 100 ? 'text-green-600' : m.achievement_pct >= 80 ? 'text-yellow-600' : 'text-red-600'}`}>{Math.round(m.achievement_pct)}%</span>
                                                    </td>
                                                    <td className={`p-1 text-center ${m.gap >= 0 ? 'text-green-600' : 'text-red-600'} ${group.bg}`}>{m.gap > 0 ? '+' : ''}{m.gap}</td>
                                                    <td className={`p-1 text-center text-gray-400 ${group.bg}`}>{m.prev_month}</td>
...
const STNotaPage: React.FC = () => {
    ...
    const availableTapOptions = useMemo(() => {
        ...
        const sourceData = filters.salesforce.length > 0
            ? sellThruNotaOutlets.filter(o => filters.salesforce.includes(o.salesforce_name))
            : sellThruNotaOutlets;
    ...
    const availableSfOptions = useMemo(() => {
        ...
        const sourceData = filters.tap.length > 0
            ? sellThruNotaOutlets.filter(o => filters.tap.includes(o.tap_name))
            : sellThruNotaOutlets;
    ...
    const locationOptions = useMemo(() => {
        ...
        sellThruNotaOutlets.forEach(o => { if (o.lokasi_outlet) locs.add(o.lokasi_outlet); });
        return Array.from(locs).sort().map(l => ({ value: l, label: l }));
    }, []);

    const filteredTAPSummary = useMemo(() => {
        if (!filters.tap || filters.tap.length === 0) return notaTAPSummary;
        return notaTAPSummary.filter(t => filters.tap!.includes(t.name));
    }, [filters.tap]);

    const filteredSFSummary = useMemo(() => {
        if (!filters.salesforce || filters.salesforce.length === 0) return notaSFSummary;
        return notaSFSummary.filter(s => filters.salesforce!.includes(s.name));
    }, [filters.salesforce]);

    const filteredOutlets = useMemo(() => {
        let result = sellThruNotaOutlets;
    ...
    return (
        <div className="p-6 animate-fade-in">
            <Header title="Sell Thru Nota" subtitle="Tracking penjualan dari nota distributor" />

            <div className="flex items-center gap-2 mt-2 text-sm text-gray-500">
                <Clock size={14} />
                <span>Data diperbarui: <span className="font-medium text-gray-700">{dataUpdateTime}</span></span>
                <span className="ml-4 px-2 py-0.5 bg-orange-100 text-orange-700 rounded text-xs flex items-center gap-1">
                    <FileText size={12} />
                    Sumber: Nota Fisik
                </span>
            </div>
